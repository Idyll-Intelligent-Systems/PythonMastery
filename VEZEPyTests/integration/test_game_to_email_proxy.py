...
async def test_chain():
    ...
    async with AsyncClient(transport=ASGITransport(app=email_app), base_url="http://127.0.0.1:8004") as email_client:
        await email_client.get("/ui/inbox")  # seed mailbox

    async with AsyncClient(transport=ASGITransport(app=game_app), base_url="http://test") as game_client:
